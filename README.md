...
Создаем RGB-лампу. Сразу в настройках лапмы редактируем минимальную и максимальную температуру. Минимаоьную ставим в `1`, максимальную в `45`. С помощью цветовой температуры мы будем управлять эффектами.
> Данный режим еще находится в тестировании, пока работает через раз. Моя Алиса не всегда понимает что я от нее хочу.
Выставляем в настройках правила следующим образом:

![image](https://user-images.githubusercontent.com/46594859/155587826-8316950f-9213-49fb-82c4-3b3150d032ea.png)

**Обратите внимание, вместо `Запрос цвета` стоит заглушка. Этот режим пока проходит дополнительное тестирование, т.к. в некоторых случаях может вызывать отсутствие реакции Алисы на ваши команды**

# TODO-лист
- [ ] Do_Output имеет влияние на лог;
- [ ] Запрос цвета работает корректно;
